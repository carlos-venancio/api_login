...
exports.get = async (req, res) => {
  try {
    ...
    if (validarTentativaDeInjecao(dados))
      res
        .status(400)
    ...
    const senhaValida =
      userSelected.password[0] !== "$"
        ? userSelected.password === dados.password
        : validarSenhaCriptografada(dados.password, userSelected.password);
    ...
    res
      .status(200)
      .send(sucessResponse(200, token, userSelected.username, "Logado"));
  } catch (e) {
...
exports.post = async (req, res) => {
  try {
    ...
    if (
      e.message.includes("E11000") & e.message.toLowerCase().includes("email")
    )
...
exports.validarRecoveryCode = async (req, res) => {
  try {
    ...
    if (req.body.recoveryCode == userSelect.recoveryCode) {
      ...
      const token = await insertResetPass.registerToken(
        userSelect._id,
        userSelect.email
      );
      ...
      res.status(400).send(simpleResponse(400, "código incorreto"));
    }
  } catch (e) {
